...
[EnabledInDm(false)]
public class Reactions : InteractionModuleBase<SocketInteractionContext>
{
    ...
    public async Task SfwAutocomplete()
    {
        ...
        var results = m_sfwCategories.Where(x => x.Name.StartsWith(userInput, StringComparison.InvariantCultureIgnoreCase)); // only send suggestions that starts with user's input; use case insensitive matching
