...
{
	private readonly RateLimiter.TimeLimiter m_firstLimit = RateLimiter.TimeLimiter.GetFromMaxCountByInterval(5, TimeSpan.FromSeconds(1));
	private readonly RateLimiter.TimeLimiter m_secondLimit = RateLimiter.TimeLimiter.GetFromMaxCountByInterval(5, TimeSpan.FromSeconds(1));
	...
	public async Task<MangaID?> GetManga(long id)
	{
		...
		if (s_cacheManga.Exists(mangaId))
		{
			return s_cacheManga.Get(mangaId);
		}
	...
	public async Task<AnimeID?> GetAnime(long id)
	{
		...
		if (s_cacheAnime.Exists(animeId))
		{
			return s_cacheAnime.Get(animeId);
		}
	...
	public async Task<Anime?> GetAnime(string name)
	{
		...
		{
			return s_cacheAnimeRaw.Get(name);
		}
